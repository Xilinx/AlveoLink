...
class XNIK_TX {
public:
    static const unsigned int t_WaitCycles = 400; //400 for hw, 10 for hw_emu
...
#pragma HLS ARRAY_PARTITION variable = m_sendSeqNoArr complete dim=0 
        switch(m_state) {
            ...
            case TX_STATE::tx_ack:
                ...
                else if (m_ackPkt.isRetran()) {
                    ap_uint<t_SeqBits> l_ackSeqNo = m_ackPkt.getSeqNo();
                    ap_uint<t_SeqBits> l_nextSeqNo = m_sendSeqNoArr[m_ackDest];
...
#pragma HLS PIPELINE II=1
                    ...
                    if (l_xnikPkt.isWorkload()) {
                        ...
                        m_waitAck = !(l_nextSeqNo < t_MaxTxSeqNo) && (l_xnikPkt.getLast() == 1);
                        m_waitDest = l_dest;
            ...
            case TX_STATE::tx_waitAck:
                if (!p_inAckStr.empty()) {
                    ...
                    PktXNIK<t_NetDataBits, t_DestBits> l_xnikPkt;
                    PktType<t_TypeBits, t_SeqBits> l_pktType(XNIK_PKT_TYPE::QUERY, 0);
...
#pragma HLS PIPELINE II=1
            ...
            if (l_xnikPkt.isData()) {
                ...
                //sender is waiting for the ack from the receiver
                AckPkt<t_NetDataBits, t_DestBits, t_TypeBits, t_SeqBits> l_retranPkt(XNIK_PKT_TYPE::RETRAN, l_expSeqNo, l_dest);
                l_retranPkt.write(p_outAckStr);
           }
        }
    }
