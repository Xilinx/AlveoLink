...
ifeq ($(TARGET),hw)
include $(XFLIB_DIR)/network/dv/hw/config.mk
endif
...
HOST_SRCS += $(XFLIB_DIR)/tests/network/dv/host/driver_main_hw_emu.cpp $(XFLIB_DIR)/common/sw/src/xNativeFPGA.cpp
CXXFLAGS +=  -D AL_netDataBits=512 -D AL_maxConnections=16 -D AL_destBits=16 -D AL_numInfs=2
CXXFLAGS +=  -I $(XFLIB_DIR)/tests/network/dv/host -I $(XFLIB_DIR)/network/dv/sw/include -I $(XFLIB_DIR)/common/sw/include
#CXXFLAGS += -O3 
CXXFLAGS += -g -O0 
...
HOST_ARGS := $(BUILD_DIR)/dvTestDriver.xclbin 1 16 

########################## Kernel compiler global settings ##########################
VPP_FLAGS +=  -D AL_mtuBytes=1472 -D AL_maxConnections=16 -D AL_netDataBits=512 -D AL_destBits=16 

VPP_FLAGS +=  -I $(XFLIB_DIR)/common/hw/include -I $(XFLIB_DIR)/kernel/hw/include -I $(XFLIB_DIR)/adapter/dv/hw/include -I $(XFLIB_DIR)/tests/network/dv/kernel

ifeq ($(INTERFACE), 0)
VPP_LDFLAGS += --config $(CUR_DIR)/conn_u55_if0_hw_emu.tmp.cfg
else ifeq ($(INTERFACE), 2)
VPP_LDFLAGS += --config $(CUR_DIR)/conn_u55_if2.cfg
endif



######################### binary container global settings ##########################
VPP_FLAGS_krnl_driver +=  -D KERNEL_NAME=krnl_driver
VPP_FLAGS_krnl_driver += --hls.clock 300000000:krnl_driver
VPP_FLAGS_krnl_xnik_wide += --hls.clock 300000000:krnl_xnik_wide
VPP_FLAGS_dv_adapter0 += --hls.clock 300000000:dv_adapter0
VPP_FLAGS_dv_adapter1 += --hls.clock 300000000:dv_adapter1
ifneq ($(HOST_ARCH), x86)
VPP_LDFLAGS_krnl_driver += --clock.defaultFreqHz 200000000 
else
VPP_LDFLAGS_krnl_driver += --kernel_frequency 200
endif

ifeq ($(HOST_ARCH), x86)
BINARY_CONTAINERS += $(BUILD_DIR)/dvTestDriver.xclbin
else
BINARY_CONTAINERS += $(BUILD_DIR)/dvTestDriver_pkg.xclbin
BINARY_CONTAINERS_PKG += $(BUILD_DIR)/dvTestDriver.xclbin
endif

# ################ Setting Rules for Binary Containers (Building Kernels) ################
$(TEMP_DIR)/krnl_driver.xo: $(XFLIB_DIR)/tests/network/dv/kernel/krnl_driver.cpp 
	$(ECHO) "Compiling Kernel: krnl_driver"
	mkdir -p $(TEMP_DIR)
	$(VPP) -c $(VPP_FLAGS_krnl_driver) $(VPP_FLAGS) -k krnl_driver -I'$(<D)' --temp_dir $(TEMP_DIR) --report_dir $(TEMP_REPORT_DIR) -o'$@' '$<'
$(TEMP_DIR)/krnl_xnik_wide.xo: $(XFLIB_DIR)/adapter/dv/hw/src/krnl_xnik_wide.cpp 
	$(ECHO) "Compiling Kernel: krnl_xnik_wide"
	mkdir -p $(TEMP_DIR)
	$(VPP) -c $(VPP_FLAGS_krnl_xnik_wide) $(VPP_FLAGS) -k krnl_xnik_wide -I'$(<D)' --temp_dir $(TEMP_DIR) --report_dir $(TEMP_REPORT_DIR) -o'$@' '$<'
$(TEMP_DIR)/dv_adapter0.xo: $(XFLIB_DIR)/tests/network/dv/kernel/dv_adapter0_hw_emu.cpp 
	$(ECHO) "Compiling Kernel: dv_adapter0"
	mkdir -p $(TEMP_DIR)
	$(VPP) -c $(VPP_FLAGS_dv_adapter0) $(VPP_FLAGS) -k dv_adapter0 -I'$(<D)' --temp_dir $(TEMP_DIR) --report_dir $(TEMP_REPORT_DIR) -o'$@' '$<'
$(TEMP_DIR)/dv_adapter1.xo: $(XFLIB_DIR)/tests/network/dv/kernel/dv_adapter1_hw_emu.cpp 
	$(ECHO) "Compiling Kernel: dv_adapter1"
	mkdir -p $(TEMP_DIR)
	$(VPP) -c $(VPP_FLAGS_dv_adapter1) $(VPP_FLAGS) -k dv_adapter1 -I'$(<D)' --temp_dir $(TEMP_DIR) --report_dir $(TEMP_REPORT_DIR) -o'$@' '$<'
BINARY_CONTAINER_krnl_driver_OBJS += $(TEMP_DIR)/krnl_driver.xo
BINARY_CONTAINER_xnik_OBJS += $(TEMP_DIR)/krnl_xnik_wide.xo
BINARY_CONTAINER_xnik_OBJS += $(TEMP_DIR)/dv_adapter0.xo
BINARY_CONTAINER_xnik_OBJS += $(TEMP_DIR)/dv_adapter1.xo
BINARY_CONTAINERS_DEPS += $(BINARY_CONTAINER_krnl_driver_OBJS)
BINARY_CONTAINERS_DEPS += $(BINARY_CONTAINER_xnik_OBJS)

$(BINARY_CONTAINERS): $(BINARY_CONTAINERS_DEPS)
	mkdir -p $(BUILD_DIR)
	$(VPP) -l $(VPP_FLAGS) --temp_dir $(TEMP_DIR) --report_dir $(BUILD_REPORT_DIR)/krnl_driver $(VPP_LDFLAGS)  $(VPP_LDFLAGS_krnl_driver) $(AIE_LDFLAGS)   -o $@ $^
