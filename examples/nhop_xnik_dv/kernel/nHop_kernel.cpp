...
void writeOut(hls::stream<ap_uint<512> >& strmIn,
              hls::stream<bool>& strmInEnd,
              hls::stream<ap_axiu<512, 0, 0, 0> >& strmOut) {
#pragma HLS INLINE off
...
#pragma HLS PIPELINE II = 1
        ...
        tmp.data = strmIn.read();
        end = strmInEnd.read();

        if (tmp.data != 0) strmOut.write(tmp);
    } while (!end);
}
...
#pragma HLS resource variable = switchMergeStreamEnd core = FIFO_LUTRAM
    ...
    writeOut(switchMergeStream, switchMergeStreamEnd, switchOut);
}
...
#pragma HLS INTERFACE axis port = switchIn bundle = control
#pragma HLS INTERFACE axis port = switchOut bundle = control
...
// std::cout << "mapTable[" << i << "]=" << mapTable[i] << std::endl;
    ...
    nHopKernelStrmWrapper(numHop, intermediate, numPairs, localBatchSize, switchBatchSize, pingPongBatchSize, hashSize,
                          ...
                          id_broadcast, id_self, blen_write, bufferFIFO0, bufferFIFO1,
