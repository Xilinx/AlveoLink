...
int main(int argc, char** argv) {
    if (argc <8  || (std::string(argv[1]) == "-help")) {
        ...
        std::cout << argv[0] << " <socket_file> <ip_file> <numPkts> <batchPkts> <timeOutCycles> <waitCycles> <flushCounter> [num_of_runs]" << std::endl;
    ...
    unsigned int l_batchPkts = atoi(argv[l_idx++]);
    unsigned int l_timeOutCycles = atoi(argv[l_idx++]);
    ...
    std::cout <<"INFO: waitCycles = " << l_waitCycles << std::endl; 
    ...
    managerHost<AL_maxConnections> l_manager;
