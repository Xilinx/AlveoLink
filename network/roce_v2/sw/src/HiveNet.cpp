...
void HiveNet::setNretranssmitions(const unsigned short retranssmitions) {
    this->writeReg(retranssmitions_off, retranssmitions);
}
uint16_t HiveNet::getNretranssmitions() {
...
uint32_t HiveNet::getIPAddress() {
    ...
    uint32_t subnet_little =
        ((subnet >> 24) & 0xff) | ((subnet << 8) & 0xff0000) | ((subnet >> 8) & 0xff00) | ((subnet << 24) & 0xff000000);
